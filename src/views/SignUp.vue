<template>
  <div>
    <h1>Sign Up</h1>
    <form @submit.prevent="onSubmit">
      <div>
        <label for="name">Name</label>
        <input type="text" v-model="name" id="name" requried />
      </div>
      <div>
        <label for="age">Age</label>
        <input type="text" v-model="age" id="age" requried />
      </div>
      <div>
        <label for="city">City</label>
        <input type="text" v-model="city" id="city" requried />
      </div>
      <div>
        <label for="job">Job</label>
        <input type="text" v-model="job" id="job" requried />
      </div>
      <div>
        <label for="email">Email</label>
        <input type="email" v-model="email" id="email" requried />
      </div>
      <div>
        <label for="pass">Password</label>
        <input type="password" v-model="pass" id="pass" requried />
      </div>
      <button type="submit">Submit</button>
    </form>
  </div>
</template>

<script>
// Dispatch an action on form submit to update the state and send the data to axios
// 1. Map the actions form Vuex
import { mapActions } from "vuex";

export default {
  data() {
    return {
      name: "",
      age: "",
      city: "",
      job: "",
      email: "",
      pass: ""
    };
  },
  // 2. Add a method to SignUp.vue to be run on form submit
  methods: {
    ...mapActions(["signUp"]),
    onSubmit() {
      const formData = {
        name: this.name,
        age: this.age,
        city: this.city,
        job: this.job,
        email: this.email,
        password: this.pass
      };
      // 3. dispatch the action "signUp", and send the user data from the form
      this.signUp(formData);
    }
  }
};
</script>

<style>
form {
  padding: 20px;
  background: lemonchiffon;
  box-shadow: -2px 2px 6px;
  max-width: 400px;
  margin: 0 auto;
}

label {
  display: block;
}

input,
button {
  padding: 5px 10px;
  margin: 5px 0;
}
</style>